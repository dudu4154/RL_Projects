...
def main(unused_argv):
    ...
    try:
        with sc2_env.SC2Env(
            map_name="Simple96",
            players=[
                sc2_env.Agent(sc2_env.Race.terran),   # 玩家 1：我方（人族）
                sc2_env.Agent(sc2_env.Race.zerg),     # 玩家 2：蟲族 Bot
            ],
            ...
            zerg_bot.setup(env.observation_spec(), env.action_spec())
                ...
                if active is ppo_agent:
                    ...
                    if ppo_step_counter % CAMERA_INTERVAL == 0:
                        ...
                        if cam_action is not None:
                            # 視角移動：我方送 cam_action，蟲族 bot 送 no_op
                            timesteps = env.step([cam_action, actions.FUNCTIONS.no_op()])
                            if timesteps[0].last():
                                break
                            continue

                # ── 主動作：我方 agent + 蟲族 bot 同時各送一個動作 ──
                my_action   = active.step(timesteps[0])
                zerg_action = zerg_bot.step(timesteps[1])
                step_actions = [my_action, zerg_action]
