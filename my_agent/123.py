...
# === RL 動作輸出定義 (AI 部分 - 大幅簡化) ===
# 0: 發呆
# 1: 全軍選取 (F2)
# 2: 攻擊移動 (A-Move)
# 3: 視角跟隨
ACTION_DO_NOTHING = 0
ACTION_SELECT_ARMY = 1
ACTION_ATTACK_MOVE = 2
ACTION_FOLLOW_CAM = 3
...
class ProtossHybridAgent:
    def __init__(self):
        # num_actions = 4
        self.model = ActorCritic(num_inputs=84*84, num_actions=4).to(DEVICE)
    ...
    def step_ai(self, obs):
        ...
        if random.random() < self.epsilon:
            action_index = torch.tensor(random.randint(0, 3)).to(DEVICE) 
    ...
    def execute_ai_action(self, action_cmd, obs):
        ...
        # === 動作 2: 攻擊移動 (A-Move) ===
        elif action_cmd == ACTION_ATTACK_MOVE:
            ...
            elif actions.FUNCTIONS.Attack_screen.id in obs.observation.available_actions:
        ...
        # === 動作 3: 鏡頭跟隨軍隊 ===
            ...
            if MOVE_CAMERA_ACTION in obs.observation.available_actions:
                if hasattr(obs.observation, 'raw_units'):
                    stalkers = [u for u in obs.observation.raw_units 
                                if u.unit_type == STALKER_ID and u.alliance == 1]
                    if stalkers:
                        mean_x = sum(u.x for u in stalkers) / len(stalkers)
                        mean_y = sum(u.y for u in stalkers) / len(stalkers)
...
# === 新增：儲存最佳模型的函數 ===
def save_best_model(agent, model_dir, episode, score):
    if not os.path.exists(model_dir):
        os.makedirs(model_dir)
    # 檔名加上 _best
    filename = os.path.join(model_dir, f"hybrid_model_best.pth")
    torch.save(agent.model.state_dict(), filename)
    print(f"★ NEW RECORD! Best model saved: {filename} (Score: {score:.4f})")

def main(argv):
    print("=== SC2 Hybrid Agent (Continuous Training) ===")
    current_time_str = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M_Hybrid_AutoRestart")
    ...
    # 嘗試讀取舊存檔 (重啟後也會讀取，確保訓練延續)
    bot_hybrid.load_model(model_dir)
    ...
    while True:
        try:
            print("\n>>> Starting StarCraft II Environment...")
            # 建立環境
            with sc2_env.SC2Env(
                map_name="Simple64",
                players=[
                ...
                # 這裡的 bot_opponent 每次重啟都重新建立一個新的比較安全
                ...
                for episode in range(TOTAL_EPISODES):
                    ...
                    while True:
                        ...
                        if time.time() - last_save_time > SAVE_INTERVAL_MINUTES * 60:
                            ...
                            last_save_time = time.time()
                            print(f"[Auto-Save] Time limit reached. Saved model.")
                        ...
                        try:
                            ...
                            # 如果是遊戲進行中的小錯誤，印出來並跳出這局
                            print(f"Game Loop Error (Inner): {e}")
                            traceback.print_exc()
                            break 
                    ...
                    try:
                        if bot_hybrid.use_ai:
                            ...
                            if total_reward > best_reward:
                                ...
                                save_best_model(bot_hybrid, model_dir, episode + 1, best_reward)
                        ...
                        print(f"⚠️ Error occurred in cleanup: {e}")
                        traceback.print_exc()

        # === 捕捉環境崩潰錯誤，並自動重啟 ===
        except KeyboardInterrupt:
            print("User interrupted. Exiting.")
            break # 只有按 Ctrl+C 才會真正結束程式
        except Exception as e:
            print(f"\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print(f"CRITICAL ERROR: StarCraft II crashed or disconnected.")
            print(f"Error details: {e}")
            print(f"Restarting environment in 5 seconds...")
            print(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
            time.sleep(5)
            # 這裡不 break，迴圈會繼續，重新執行 with sc2_env...
