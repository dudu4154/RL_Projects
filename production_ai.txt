...
class ProductionAI:
    ...
    def _is_scv_selected(self, obs):
        valid_scv_ids = [45, 104, 105, 106, 107]
        
        # 1. 檢查單一選取
        if len(obs.observation.single_select) > 0:
            return obs.observation.single_select[0].unit_type == SCV_ID  # 🚨 只要不是空手，一律回傳 False 並中斷！
        
        # 檢查複數選取：同樣使用 len()
        if len(obs.observation.single_select) > 0:
            return obs.observation.multi_select[0].unit_type in valid_scv_ids
            ...
            if self.locked_action == 42:
                ...
                if self._is_scv_selected(obs):
                    ...
                    if actions.FUNCTIONS.Harvest_Gather_screen.id in available:
                        ...
                        # 如果 UI 出來了但還不能點 (例如瓦斯廠正在蓋，系統判定不能右鍵)，保持鎖定繼續等
                        return actions.FUNCTIONS.no_op()
...
def main(argv):
    ...
    sc2_env.Bot(sc2_env.Race.terran, sc2_env.Difficulty.very_easy) # 改為電腦 AI，難度最簡單
        ...
        while True:
            ...
            obs_list = env.reset()
            while True:
                action_id = random.randint(11,42)#14 #random.choice([1,47])#1
