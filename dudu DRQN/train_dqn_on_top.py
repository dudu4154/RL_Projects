...
import production_ai as production_ai
...
def main(argv):
    ...
    # 菁英記憶：改成 List，用來做排行榜
    success_memory = [] 
    MAX_ELITE_MEMORY = 50 # 設定排行榜最多收錄 1000 局
    ...
    def draw_dual_head_network(surface, model, state_vector, allowed_indices, hidden_state, q_values):
        import numpy as np
        import torch
        import pygame
    ...
    def draw_full_hud(surface, obs, brain_model, state_vector, allowed_indices, hidden_state, valid_actions, q_values):
        ...
        # (原本這裡的 with torch.no_grad(): 區塊已經被刪除了！)
               ...
               # --- ✨ 統一計算真實的建築數量 (基於執行動作前的畫面 s_unit) ---
                ...
                if RENDER_UI:
                    ...
                    if train_step_counter % 5 == 0:
                        # ✨ 修正 2：將大腦剛算出來的 q_actions 轉成 numpy 傳給 UI
                        current_q_vals = q_actions.cpu().detach().numpy()[0]
                        draw_full_hud(
                            screen, obs, brain_model, state,
                            allowed_indices, hidden_state, VALID_ACTIONS, current_q_vals
                        )
                        pygame.display.flip() # ✨ 確保畫完後有刷新螢幕
                ...
                # 替換為以下邏輯：
                # train_dqn_on_top.py (約 1410 行處)

                # 1. 先取出舊的數量，如果沒有就預設為 0
                # train_dqn_on_top.py (約 1410 行處)

                # 1. 先取出舊的數量，如果沒有就預設為 0
                prev_count = getattr(agent, 'last_target_count', 0)

                # 2. 進行比較
                if current_real_count > prev_count:
                    # 3. 使用剛剛取出的局部變數 prev_count 進行減法，避免 AttributeError
                    diff = current_real_count - prev_count
                ...
                if current_action_success == 1.0:
                    ...
                    # ✅ 正確寫法：把 hidden_state 搬回 CPU
                    failed_transition = (
                        state, action_index, p_id - 1, scaled_reward, next_state, done, 
                        (hidden_state[0].cpu(), hidden_state[1].cpu()), 
                        (next_hidden_state[0].cpu(), next_hidden_state[1].cpu()), 
                        0.0, next_allowed_indices, current_action_success
                    )
                ...
                if done:
                    ...
                    if current_real_count >= 5:
                        # ✨ 修正 3：加入破紀錄檢查，避免無意義的排序與寫入
                        is_new_record = False
                        if len(success_memory) == 0 or current_loop < success_memory[0][0]:
                            is_new_record = True
                            
                        success_memory.append((current_loop, episode_memory))
                        
                        # 只有在剛好滿 5 的倍數，或者是打破紀錄時，才執行排序和寫入
